...
rmi = camel_folder_summary_get (vmi->orig_summary, CAMEL_MESSAGE_INFO_BASE (mi)->uid + 8);
...
vee_info_uint32 (const CamelMessageInfo *mi,
                 gint id)
{
	CamelMessageInfo *rmi = camel_folder_summary_get (((CamelVeeMessageInfo *) mi)->orig_summary, CAMEL_MESSAGE_INFO_BASE (mi)->uid + 8);
	guint32 ret;
...
{
	gint res = FALSE;
	CamelVeeFolder *vf = (CamelVeeFolder *) camel_folder_summary_get_folder (CAMEL_MESSAGE_INFO_BASE (mi)->summary);

	if (CAMEL_MESSAGE_INFO_BASE (mi)->uid) {
		CamelMessageInfo *rmi = camel_folder_summary_get (((CamelVeeMessageInfo *) mi)->orig_summary, CAMEL_MESSAGE_INFO_BASE (mi)->uid + 8);
		...
		if (ignore_changes)
			camel_vee_folder_ignore_next_changed_event (vf, camel_folder_summary_get_folder (CAMEL_MESSAGE_INFO_BASE (rmi)->summary));
		...
		if (ignore_changes) {
			if (res)
				...
				camel_vee_folder_remove_from_ignore_changed_event (vf, camel_folder_summary_get_folder (CAMEL_MESSAGE_INFO_BASE (rmi)->summary));
		}
...
{
	gint res = FALSE;
	CamelVeeFolder *vf = CAMEL_VEE_FOLDER (camel_folder_summary_get_folder (CAMEL_MESSAGE_INFO_BASE (mi)->summary));
	...
	if (CAMEL_MESSAGE_INFO_BASE (mi)->uid) {
		...
		if (res) {
			...
			camel_folder_summary_replace_flags (CAMEL_MESSAGE_INFO_BASE (mi)->summary, mi);
		}
                       ...
                       const gchar *uid)
{
	...
	if (!info) {
		...
		CAMEL_MESSAGE_INFO_BASE (info)->dirty = FALSE;
